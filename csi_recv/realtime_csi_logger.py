...
# -*- coding: utf-8 -*-
# 采集:
#   python .\realtime_csi_logger.py -p COM7 -b 921600 -o .\capture_run_01 --collect-training-data
# 推理:
#   python .\realtime_csi_logger.py -p COM7 -b 921600 -o .\capture_run_01 --model-path .\csi_hr.keras
...
新增上位机时间字段：
  host_rx_us   : 上位机收到这一行串口数据时的本地微秒时间戳
  host_rx_iso  : 上位机收到这一行串口数据时的 ISO 时间字符串
"""
...
class Stats:
    def __init__(self) -> None:
        self.raw_lines = 0
        self.espnow_lines = 0
        self.csi_meta_lines = 0
        self.csi_data_lines = 0
        self.bad_lines = 0
        self.bad_csi_lines = 0
        self.last_espnow_seq: Optional[int] = None
        self.espnow_missing = 0
        self.last_print_time = time.time()
        self.last_csi_count = 0
        self.last_espnow_count = 0
        self.pred_count = 0
    ...
    def maybe_print(self, interval: float) -> None:
        ...
        espnow_delta = self.espnow_lines - self.last_espnow_count
        csi_rate = csi_delta / dt if dt > 0 else 0.0
        espnow_rate = espnow_delta / dt if dt > 0 else 0.0

        print(
            f"[{datetime.now().strftime('%H:%M:%S')}] "
            f"CSI={self.csi_data_lines} ({csi_rate:.2f}/s), "
            f"ESPNOW={self.espnow_lines} ({espnow_rate:.2f}/s), "
            f"META={self.csi_meta_lines}, pred={self.pred_count}, "
            f"bad={self.bad_lines}, bad_csi={self.bad_csi_lines}, "
            f"espnow_missing={self.espnow_missing}"
        )

        self.last_print_time = now
        self.last_csi_count = self.csi_data_lines
        self.last_espnow_count = self.espnow_lines
...
def build_stats_summary(stats: "Stats") -> dict[str, int]:
    return {
        ...
        "espnow_lines": stats.espnow_lines,
        "csi_meta_lines": stats.csi_meta_lines,
        "csi_data_lines": stats.csi_data_lines,
        "bad_lines": stats.bad_lines,
        "bad_csi_lines": stats.bad_csi_lines,
        "espnow_missing": stats.espnow_missing,
        "pred_count": stats.pred_count,
    }


def parse_espnow_line(line: str) -> list[str]:
    row = next(csv.reader(StringIO(line)))
    if len(row) != 5:
        raise ValueError(f"ESPNOW_DATA field count mismatch: {len(row)}")
    int(row[1])
    int(row[2])
    int(row[3])
    return row
...
def parse_csi_meta_line(line: str) -> list[str]:
    row = next(csv.reader(StringIO(line)))
    if len(row) != 5:
        raise ValueError(f"CSI_META field count mismatch: {len(row)}")
    int(row[1])
    int(row[2])
    int(row[3])
    int(row[4])
...
def main() -> None:
    parser = argparse.ArgumentParser(description="实时读取、保存并处理 ESP32 CSI 串口输出")
    ...
    parser.add_argument("--training-file", default="training_data.txt")
    ...
    raw_log_path = os.path.join(output_dir, build_tagged_filename(session_id, "raw_serial.log"))
    bad_log_path = os.path.join(output_dir, build_tagged_filename(session_id, "bad_lines.log"))
    espnow_csv_path = os.path.join(
        output_dir, build_tagged_filename(session_id, "espnow_data.csv")
    )
    meta_csv_path = os.path.join(output_dir, build_tagged_filename(session_id, "csi_meta.csv"))
    csi_csv_path = os.path.join(output_dir, build_tagged_filename(session_id, "csi_data.csv"))
    training_path = os.path.join(
        output_dir, build_tagged_filename(session_id, args.training_file)
    )

    raw_log = open(raw_log_path, "a", encoding="utf-8")
    bad_log = open(bad_log_path, "a", encoding="utf-8")
    espnow_f, espnow_writer = open_csv_with_header(espnow_csv_path, ESPNOW_HEADER)
    meta_f, meta_writer = open_csv_with_header(meta_csv_path, CSI_META_HEADER)
    csi_f, csi_writer = open_csv_with_header(csi_csv_path, CSI_DATA_HEADER)

    train_f = None
    model = None
    if args.collect_training_data:
        train_f = open(training_path, "a", encoding="utf-8")
        print(f"Training mode ON -> {training_path}")
    else:
    ...
    print(f"bad lines -> {bad_log_path}")
    print(f"espnow    -> {espnow_csv_path}")
    print(f"meta      -> {meta_csv_path}")
    ...
    try:
        while True:
            ...
            stats.raw_lines += 1
            raw_log.write(f"{host_rx_us},{host_rx_iso},{line}\n")
            raw_log.flush()

            try:
                if line.startswith("ESPNOW_DATA,"):
                    row = parse_espnow_line(line)

                    seq = int(row[1])
                    if (
                        stats.last_espnow_seq is not None
                        and seq > stats.last_espnow_seq + 1
                    ):
                        stats.espnow_missing += seq - stats.last_espnow_seq - 1
                    stats.last_espnow_seq = seq

                    espnow_writer.writerow(row + [str(host_rx_us), host_rx_iso])
                    espnow_f.flush()
                    stats.espnow_lines += 1

                elif line.startswith("CSI_META,"):
                    row = parse_csi_meta_line(line)
                    meta_writer.writerow(row + [str(host_rx_us), host_rx_iso])
                    meta_f.flush()
                    stats.csi_meta_lines += 1
                    ...
                    if args.collect_training_data:
                        train_f.write(",".join(f"{x:.6f}" for x in shaped) + "\n")
                        train_f.flush()
                    else:
                    ...
                    bad_log.write(f"{host_rx_us},{host_rx_iso},{line}\n")
                    bad_log.flush()
                    stats.bad_lines += 1

            except Exception as exc:
                bad_log.write(
                    f"[PARSE_ERROR],{host_rx_us},{host_rx_iso},{type(exc).__name__},{exc}\n"
                )
                bad_log.write(f"{host_rx_us},{host_rx_iso},{line}\n")
                bad_log.flush()
        ...
        if (
            ...
            session_metadata["files"] = {
                "raw_log": os.path.abspath(raw_log_path),
                "bad_log": os.path.abspath(bad_log_path),
                "espnow_csv": os.path.abspath(espnow_csv_path),
                "csi_meta_csv": os.path.abspath(meta_csv_path),
                "csi_data_csv": os.path.abspath(csi_csv_path),
            }
            if train_f is not None:
                session_metadata["files"]["training_data"] = os.path.abspath(
                    training_path
                )
            write_session_metadata(session_meta_path, session_metadata)
